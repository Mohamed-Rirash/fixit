defmodule FixitWeb.TeamLiveTest do
  use FixitWeb.ConnCase

  import Phoenix.LiveViewTest
  import Fixit.WorkspacesFixtures

  @invalid_attrs %{name: nil, role: "backend", organization_id: nil}
  ...
  defp organization_refs(%{scope: scope}) do
    organization = organization_fixture(scope)

    %{
      create_attrs: %{name: "some name", role: "backend", organization_id: organization.id},
      update_attrs: %{
  ...
  describe "Index" do
    setup [:create_team, :organization_refs]
    ...
    test "saves new team", %{conn: conn, create_attrs: create_attrs} do
    ...
    test "updates team in listing", %{conn: conn, team: team, update_attrs: update_attrs} do
      {:ok, index_live, _html} = live(conn, ~p"/teams")
  ...
  describe "Show" do
    ...
    test "updates team and returns to show", %{conn: conn, team: team, update_attrs: update_attrs} do
